...
public class Model {
	...
	private void cerca2(Set<Esame> parziale, int L, int m) {
		...
		if(sommaCrediti==m) {
			...
			if(mediaVoti>mediaMigliore) {
				migliore = new HashSet<Esame>(parziale); //facciamo una copia
	...
	/*
	 *  COMPLESSITA' N!
	 *  IL LIVELLO DELLA RICORSIONE COINCIDE CON GLI ESAMI MESSI IN PARZIALE
	 */
	private void cerca1(Set<Esame> parziale, int L, int m) {
		...
		for(Esame e : esami) {
			if(!parziale.contains(e)) {
				...
				parziale.remove(e); //con le liste il backtracking si fa togliendo l'ultimo elemento
									//perche potrebbero esserci duplicati
			}
		}
